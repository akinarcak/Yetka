...
SECURITY_SERVICE_SIGNATURE_ALLOW_LEGACY = CONFIG.SECURITY_SERVICE_SIGNATURE_ALLOW_LEGACY
SECURITY_SERVICE_SIGNATURE_WINDOW_SECONDS = CONFIG.SECURITY_SERVICE_SIGNATURE_WINDOW_SECONDS
...
SSH_KNOWN_HOSTS_FILE = CONFIG.SSH_KNOWN_HOSTS_FILE
SSH_ALLOW_UNPINNED_HOST_KEYS = CONFIG.SSH_ALLOW_UNPINNED_HOST_KEYS
SSH_LEGACY_CRYPTO_ENABLED = CONFIG.SSH_LEGACY_CRYPTO_ENABLED
...
CLOUD_SYNC_ALLOWED_ENDPOINTS = tuple(
    endpoint.strip().rstrip('/')
    for endpoint in CONFIG.CLOUD_SYNC_ALLOWED_ENDPOINTS.split(',')
    if endpoint.strip()
)
...
# arcak-pam: community sürümünde varlık sayısı sınırı. 0 = sınırsız (açık kaynak varsayılanı).
ASSET_COMMUNITY_LIMIT = int(CONFIG.ASSET_COMMUNITY_LIMIT or 0)
